...
void ClickGui::renderCategory(Category category) {
	...
	auto interfaceColor = ColorUtil::interfaceColor(3);
	...
	if (ourWindow->isExtended || ourWindow->isInAnimation) {
		...
		for (auto& mod : moduleList) {
			...
			if (allowRender) {
				if (!ourWindow->isInAnimation && !isDragging && rectPos.contains(&mousePos)) {  // Is the Mouse hovering above us?
					DrawUtils::fillRectangle(rectPos, interfaceColor, backgroundAlpha);//鼠标预选
					...
					if (DrawUtils::shouldToggleLeftClick && !ourWindow->isInAnimation) {  // Are we being clicked?//被点击动画
						mod->toggle();
						//DrawUtils::fillRectangle(rectPos, interfaceColor,backgroundAlpha);
						DrawUtils::shouldToggleLeftClick = false;
					}
				} else {
					DrawUtils::fillRectangle(rectPos, mod->isEnabled() ? interfaceColor : ClientColors::clickGuiModuleColor, backgroundAlpha);  // 子项
				...
				if (settings->size() > 2 && allowRender) {
					...
					if (clickMod->isExtended) {
						...
						if (endYOffset - startYOffset > textHeight + 1 || overflowing) {
							...
							//DrawUtils::drawRoundRectangle(rectPos2, ClientColors::clickGuiModuleColor, true);
						}
					}
				} else
					currentYOffset += textHeight + (textPaddingY * 2);
		...
		DrawUtils::drawRoundRectangle(winRectPos, MC_Color(30,30,30,200), false);
	...
	const float xOffset2 = ourWindow->pos.x;
	const float yOffset2 = ourWindow->pos.y;
	{
		Vec2 textPos = Vec2(
			currentXOffset + textPaddingX,
		...
		Vec4 rectPos2 = Vec4(
			currentXOffset - categoryMargin + 1,
			categoryHeaderYOffset - categoryMargin,
			currentXOffset + windowSize->x + paddingRight + categoryMargin - 1,
			// categoryHeaderYOffset + textHeight + (textPaddingY * 2));
			categoryHeaderYOffset + moduleList.size() * (textHeight + (textPaddingY * 3)));
		...
		// Draw category component
		{
			...
			DrawUtils::fillRoundRectangle(Vec4(rectPos.x, rectPos.y, rectPos.z, rectPos.w - 2), MC_Color(30, 30, 30, 230), 1.f);
