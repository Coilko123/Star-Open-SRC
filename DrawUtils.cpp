...
Font* DrawUtils::getFont(Fonts font) {
	static auto fontChangerModule = moduleMgr->getModule<HudModule>();
	...
	if (fontChangerModule->Fonts.selected == 4)
		return Game.getClientInstance()->minecraftGame->getTheRandomFontPlease();
...
void DrawUtils::drawCenteredString(Vec2 pos, std::string* textStr, float textSize, MC_Color color, bool hasShadow, float alpha) {
	DrawUtils::drawText(Vec2(pos.x - DrawUtils::getTextWidth(textStr, textSize) / 2.F, pos.y - DrawUtils::getFont(Fonts::SMOOTH)->getLineHeight() / 2.F), textStr, color, textSize, alpha, hasShadow);
}

void DrawUtils::drawText(Vec2 pos, std::string* textStr, MC_Color color, float textSize, float alpha, bool hasShadow, Fonts font) {
	TextHolder text(*textStr);
	Font* fontPtr = getFont(font);
...
void DrawUtils::drawBox(const Vec3& lower, const Vec3& upper, float lineWidth, bool fill, int mode, bool enableLerp, float lerpDuration, float elapsedLerpTime) {
	// Lerp
	Vec3 lerpedLower, lerpedUpper;
	...
	Vec3 diff = upper.sub(lerpedLower);

	// Create an array of vertices representing the corners of the box
	Vec3 vertices[8];
	vertices[0] = Vec3(lerpedLower.x, lerpedLower.y, lerpedLower.z);
	vertices[1] = Vec3(lerpedLower.x + diff.x, lerpedLower.y, lerpedLower.z);
	vertices[2] = Vec3(lerpedLower.x, lerpedLower.y + diff.y, lerpedLower.z);
	vertices[3] = Vec3(lerpedLower.x + diff.x, lerpedLower.y + diff.y, lerpedLower.z);
	vertices[4] = Vec3(lerpedLower.x, lerpedLower.y, lerpedLower.z + diff.z);
	vertices[5] = Vec3(lerpedLower.x + diff.x, lerpedLower.y, lerpedLower.z + diff.z);
	vertices[6] = Vec3(lerpedLower.x, lerpedLower.y + diff.y, lerpedLower.z + diff.z);
	vertices[7] = Vec3(lerpedLower.x + diff.x, lerpedLower.y + diff.y, lerpedLower.z + diff.z);
...
void DrawUtils::drawNameTags(Entity* ent, float textSize, bool drawHealth, bool useUnicodeFont) {
	...
	if (refdef->OWorldToScreen(origin, ent->getRenderPositionComponent()->renderPos.add(0, 0.5f, 0), textPos, fov, screenSize)) {
		...
		drawText(textPos, &text, MC_Color(255, 255, 255), textSize,1,true);
...
void DrawUtils::drawRoundRectangle(Vec4 pos, const MC_Color col, bool rounder = false) {
	switch (rounder) {
	case 0:
		DrawUtils::fillRectangleA(Vec4(pos.x + 1, pos.y - 2, pos.z - 1, pos.y - 1), col);
		DrawUtils::fillRectangleA(Vec4(pos.x + 1, pos.w + 1, pos.z - 1, pos.w + 2), col);
		DrawUtils::fillRectangleA(Vec4(pos.x - 1, pos.y, pos.x, pos.w), col);
		DrawUtils::fillRectangleA(Vec4(pos.z, pos.y, pos.z + 1, pos.w), col);
		DrawUtils::fillRectangleA(Vec4(pos.z - 1, pos.w, pos.z, pos.w + 1), col);
		DrawUtils::fillRectangleA(Vec4(pos.x, pos.w, pos.x + 1, pos.w + 1), col);
		DrawUtils::fillRectangleA(Vec4(pos.z - 1, pos.y - 1, pos.z, pos.y), col);
		DrawUtils::fillRectangleA(Vec4(pos.x, pos.y - 1, pos.x + 1, pos.y), col);
		break;
	case 1:
		DrawUtils::fillRectangleA(Vec4(pos.x - 1, pos.y, pos.x, pos.w), col);
		DrawUtils::fillRectangleA(Vec4(pos.z, pos.y, pos.z + 1, pos.w), col);
		DrawUtils::fillRectangleA(Vec4(pos.x + 2, pos.w + 2, pos.z - 2, pos.w + 3), col);
		DrawUtils::fillRectangleA(Vec4(pos.x + 2, pos.y - 3, pos.z - 2, pos.y - 2), col);
		DrawUtils::fillRectangleA(Vec4(pos.x, pos.y - 2, pos.x + 1, pos.y - 1), col);
		DrawUtils::fillRectangleA(Vec4(pos.x + 1, pos.y - 2.5, pos.x + 2, pos.y - 1.5), col);
		DrawUtils::fillRectangleA(Vec4(pos.x - .5, pos.y - 1, pos.x + .5, pos.y), col);
		DrawUtils::fillRectangleA(Vec4(pos.z - 1, pos.y - 2, pos.z, pos.y - 1), col);
		DrawUtils::fillRectangleA(Vec4(pos.z - 2, pos.y - 2.5, pos.z - 1, pos.y - 1.5), col);
		DrawUtils::fillRectangleA(Vec4(pos.z - .5, pos.y - 1, pos.z + .5, pos.y), col);
		DrawUtils::fillRectangleA(Vec4(pos.x - .5, pos.w, pos.x + .5, pos.w + 1), col);
		DrawUtils::fillRectangleA(Vec4(pos.x, pos.w + 1, pos.x + 1, pos.w + 2), col);
		DrawUtils::fillRectangleA(Vec4(pos.x + 1, pos.w + 1.5f, pos.x + 2, pos.w + 2.5), col);
		DrawUtils::fillRectangleA(Vec4(pos.z - .5, pos.w, pos.z + .5, pos.w + 1), col);
		DrawUtils::fillRectangleA(Vec4(pos.z - 1, pos.w + 1, pos.z, pos.w + 2), col);
		DrawUtils::fillRectangleA(Vec4(pos.z - 2, pos.w + 1.5f, pos.z - 1, pos.w + 2.5), col);
		break;
	}
}
void DrawUtils::fillRoundRectangle(Vec4 pos, const MC_Color col, bool rounder = false) {
	switch (rounder) {
	case 0:
		DrawUtils::setColor(col.r, col.g, col.b, col.a);
		DrawUtils::drawQuad({pos.x, pos.w}, {pos.z, pos.w}, {pos.z, pos.y}, {pos.x, pos.y});
		DrawUtils::fillRectangleA(Vec4(pos.x + 1, pos.y - 1, pos.z - 1, pos.y), col);
		DrawUtils::fillRectangleA(Vec4(pos.x + 1, pos.w, pos.z - 1, pos.w + 1), col);
		break;
	case 1:
		DrawUtils::fillRectangleA(Vec4(pos.x, pos.y, pos.z, pos.w), col);
		DrawUtils::fillRectangleA(Vec4(pos.x + .5, pos.w, pos.z - .5, pos.w + 1), col);
		DrawUtils::fillRectangleA(Vec4(pos.x + 1, pos.w + 1, pos.z - 1, pos.w + 1.5), col);
		DrawUtils::fillRectangleA(Vec4(pos.x + 2, pos.w + 1.5, pos.z - 2, pos.w + 2), col);
		DrawUtils::fillRectangleA(Vec4(pos.x + .5, pos.y - 1, pos.z - .5, pos.y), col);
		DrawUtils::fillRectangleA(Vec4(pos.x + 1, pos.y - 1.5f, pos.z - 1, pos.y - 1), col);
		DrawUtils::fillRectangleA(Vec4(pos.x + 2, pos.y - 2, pos.z - 2, pos.y - 1.5), col);
		break;
	}
}
...
void DrawUtils::drawGradientText(Vec2 pos, std::string* textStr, float textSize, float alpha, bool hasShadow) {
	auto interface = moduleMgr->getModule<ClickGuiMod>();
	TextHolder text(*textStr);
...
void DrawUtils::drawItemInWorld(ItemStack* item, const Vec3& location, float opacity, float scale, bool isEnchanted) {
	Vec2 itemPos;
	Vec3 actualLocation = location;
